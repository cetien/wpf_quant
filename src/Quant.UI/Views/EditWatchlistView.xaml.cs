...
public partial class EditWatchlistView : UserControl
{
    ...
    {
        var dlg = new WatchlistEditDialog(DbPath, null);
    ...
    private void BtnEdit_Click(object sender, RoutedEventArgs e)
    {
        ...
        var dlg = new WatchlistEditDialog(DbPath, row);
        if (dlg.ShowDialog() == true) LoadData();
    }
    ...
    private void BtnDelete_Click(object sender, RoutedEventArgs e)
    {
        ...
        var id   = row["watchlist_id"]?.ToString() ?? "";
        var res  = MessageBox.Show($"워치리스트 [{name}]을 삭제하시겠습니까?\n(items도 함께 삭제됩니다)",
                                   "삭제 확인", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (res != MessageBoxResult.Yes) return;
        try
        {
            Execute($"DELETE FROM watchlist_items WHERE watchlist_id = {id}");
            Execute($"DELETE FROM watchlists WHERE watchlist_id = {id}");
